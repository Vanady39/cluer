...
> **Реализация отступает от этого пункта.** Фактический формат — плоский объект из двух
> строк, без `code` и без массива `details`:
>
> ```json
> {
>   "error": "tour cannot be published: hints[1].content: must not be empty",
>   "message": "Tour cannot be published"
> }
> ```
>
> Класс ошибки несёт HTTP-статус, а привязка к полю читается из `error`, где каждая
> проблема записана как `путь: причина`, а несколько склеиваются через `; `. Актуальное
> описание — в [onboarding-api.md](onboarding-api.md), раздел «Формат ошибок».
