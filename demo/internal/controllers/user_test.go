...
func userRouter(t *testing.T, claims *oidcauth.Claims) *gin.Engine {
	t.Helper()
...
func TestUserController_GetCurrentUser(t *testing.T) {
	...
	t.Run("profile claims the IdP withheld come back empty", func(t *testing.T) {
		rec := doJSON(t, userRouter(t, &oidcauth.Claims{Subject: "ak-visitor-2"}), http.MethodGet, "/users/me", nil)
