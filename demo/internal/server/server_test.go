...
func newDemoTestServer(t *testing.T) *serve.Server {
	t.Helper()
	...
	logger := zerolog.Nop()
	srv := NewServer(&config.ServerConfig{Address: "127.0.0.1:0"}, &CreateStruct{
		Logger:            &logger,
...
func demoCall(srv *serve.Server, method, target, authorization string) *httptest.ResponseRecorder {
