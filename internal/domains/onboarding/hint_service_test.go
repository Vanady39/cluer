...
func TestHintService_Create(t *testing.T) {
	tests := []struct {
		name       string
		tourId     uuid.UUID
		input      *Hint
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			svc := NewHintService(tourRepo, hintRepo)
...
func TestHintService_ensureTourEditable(t *testing.T) {
	tests := []struct {
		name      string
		tourId    uuid.UUID
		setupTour *Tour
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			svc := NewHintService(tourRepo, hintRepo)
			result, err := svc.ensureTourEditable(context.Background(), tt.tourId)
			...
			if tt.wantErr != nil {
				...
				assert.Equal(t, tt.tourId, result.Id)
			}
		})
	}
}
