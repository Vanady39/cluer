...
// errRepoFailure stands in for an arbitrary repository failure. Tests match it by
// identity, so the same value must be used for both the injected and expected error.
...
func TestTourDomain_Create(t *testing.T) {
	tests := []struct {
		name       string
		input      *Tour
		wantErr    error
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			svc := NewTourDomain(tourRepo, hintRepo)
...
func TestTourDomain_ListPublished(t *testing.T) {
	tests := []struct {
		name       string
		path       string
		setupTours []*Tour
