...
func (tr *TourRepository) CreateWithDraft(ctx context.Context, t *domains.Tour) (*domains.Tour, error) {
	...
	if err != nil {
		return nil, wrap(err, domains.ErrTourNotFound, "", errs.Create)
	}
	...
	if err != nil {
		return nil, wrap(err, domains.ErrTourNotFound, t.Title, errs.Create)
	}

	triggerConfigBytes, err := marshalTriggerConfig(t.TriggerConfig)
	if err != nil {
		return nil, wrap(err, domains.ErrVersionNotFound, t.Id.String(), errs.Create)
	}

	var audienceRulesBytes []byte
	if t.Audience != nil && len(t.Audience.Rules) > 0 {
		audienceRulesBytes, err = marshalAudienceRules(t.Audience.Rules)
		if err != nil {
			return nil, wrap(err, domains.ErrVersionNotFound, t.Id.String(), errs.Create)
		}
	}
	...
	if err := tx.Commit(ctx); err != nil {
		return nil, wrap(err, domains.ErrTourNotFound, t.Id.String(), errs.Create)
	}
...
func (tr *TourRepository) GetById(ctx context.Context, id uuid.UUID) (*domains.Tour, error) {
	...
	if err != nil {
		return nil, wrap(err, domains.ErrTourNotFound, id.String(), errs.Retrieve)
	}
	return t, nil
}
...
func (tr *TourRepository) List(ctx context.Context, appId uuid.UUID) ([]*domains.Tour, error) {
	...
	if err != nil {
		return nil, wrap(err, domains.ErrTourNotFound, appId.String(), errs.Retrieve)
	}
	defer rows.Close()
...
func (tr *TourRepository) UpdateMeta(ctx context.Context, id uuid.UUID, p domains.TourMetaPatch) (*domains.Tour, error) {
	...
	if err != nil {
		return nil, wrap(err, domains.ErrTourNotFound, id.String(), errs.Update)
	}
	return t, nil
}
...
func (tr *TourRepository) Archive(ctx context.Context, id uuid.UUID) error {
	...
	if err != nil {
		return wrap(err, domains.ErrTourNotFound, id.String(), errs.Delete)
	}
	if tag.RowsAffected() == 0 {
		return errs.NotFound(domains.ErrTourNotFound, id.String())
	}
	return nil
}
...
func (tr *TourRepository) VersionOwner(ctx context.Context, versionId uuid.UUID) (uuid.UUID, uuid.UUID, error) {
	...
	if err != nil {
		return uuid.Nil, uuid.Nil, wrap(err, domains.ErrVersionNotFound, versionId.String(), errs.Retrieve)
	}
...
func (tr *TourRepository) ListVersions(ctx context.Context, tourId uuid.UUID) ([]*domains.TourVersion, error) {
	...
	if err != nil {
		return nil, wrap(err, domains.ErrVersionNotFound, tourId.String(), errs.Retrieve)
	}
	defer rows.Close()
...
func (tr *TourRepository) VersionByStatus(ctx context.Context, tourId uuid.UUID, s domains.TourStatus) (*domains.TourVersion, error) {
	...
	if err != nil {
		return nil, wrap(err, domains.ErrVersionNotFound, tourId.String()+"/"+string(s), errs.Retrieve)
	}
	return v, nil
}
...
func (tr *TourRepository) UpdateDraft(ctx context.Context, versionId uuid.UUID, p domains.DraftPatch) (*domains.TourVersion, error) {
	...
	if p.Audience != nil {
		...
		rulesBytes, err := marshalAudienceRules(p.Audience.Rules)

		if err != nil {
			return nil, wrap(err, domains.ErrVersionNotFound, versionId.String(), errs.Update)
		}
	...
	if p.TriggerConfig != nil {
		configBytes, err := marshalTriggerConfig(p.TriggerConfig)
		if err != nil {
			return nil, wrap(err, domains.ErrVersionNotFound, versionId.String(), errs.Update)
		}
...
func (tr *TourRepository) CopyToDraft(ctx context.Context, tourId, sourceVersionId uuid.UUID) (*domains.TourVersion, error) {
	tx, err := tr.pool.Begin(ctx)
	if err != nil {
		return nil, wrap(err, domains.ErrVersionNotFound, tourId.String(), errs.Create)
	}
	defer tx.Rollback(ctx)
	...
	if err != nil {
		return nil, wrap(err, domains.ErrVersionNotFound, sourceVersionId.String(), errs.Create)
	}
	...
	if err != nil {
		return nil, wrap(err, domains.ErrHintNotFound, v.Id.String(), errs.Create)
	}

	if err := tx.Commit(ctx); err != nil {
		return nil, wrap(err, domains.ErrVersionNotFound, v.Id.String(), errs.Create)
	}
	return v, nil
}
...
func (tr *TourRepository) Publish(ctx context.Context, tourId uuid.UUID) (*domains.TourVersion, error) {
	tx, err := tr.pool.Begin(ctx)
	if err != nil {
		return nil, wrap(err, domains.ErrTourNotFound, tourId.String(), errs.Update)
	}
	defer tx.Rollback(ctx)
	...
	if _, err := tx.Exec(ctx, `
		...
		return nil, wrap(err, domains.ErrVersionNotFound, tourId.String(), errs.Update)
	}
	...
	if err != nil {
		return nil, wrap(err, domains.ErrNoDraft, tourId.String(), errs.Update)
	}
...
func (tr *TourRepository) ResolveCandidates(ctx context.Context, appId uuid.UUID) ([]*domains.Tour, error) {
	...
	for rows.Next() {
		...
		if err := unmarshalAudienceRules(audienceRulesBytes, &t.Audience.Rules); err != nil {
			tr.logger.Error().
				Err(err).
		...
		if err := unmarshalTriggerConfig(triggerConfigBytes, &t.TriggerConfig); err != nil {
			tr.logger.Error().
				Err(err).
				Str("tour_id", t.Id.String()).
				Str("version_id", t.VersionId.String()).
...
func scanVersion(row scanner) (*domains.TourVersion, error) {
	...
	if err := unmarshalTriggerConfig(triggerConfigBytes, &v.TriggerConfig); err != nil {
		return nil, fmt.Errorf("scanVersion trigger_config (version %s): %w", v.Id, err)
	}
	if err := unmarshalAudienceRules(audienceRulesBytes, &v.Audience.Rules); err != nil {
